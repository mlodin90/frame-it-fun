...
const services = [
  { icon: Heart, title: "Weddings", description: "Elegant booths designed to complement your big day with custom backdrops and printed keepsakes." },
  { icon: Briefcase, title: "Corporate Events", description: "Branded photo experiences that elevate galas, conferences, and team celebrations." },
  { icon: Sparkles, title: "Brand Activations", description: "Turn-key booths with logo overlays and custom data capture for marketing events." },
  { icon: PartyPopper, title: "Holiday Parties", description: "Festive setups, themed props, and seasonal backdrops that bring the holiday spirit." },
  { icon: Gift, title: "Birthdays & Sweet 16s", description: "High-energy booths with luxe lighting designed for unforgettable celebrations." },
  { icon: Baby, title: "Baby Showers & Reveals", description: "Soft-styled booths perfect for showers, gender reveals, and milestone moments." },
  { icon: Cake, title: "School & Public Events", description: "Crowd-friendly setups, fast prints, and an attendant who keeps the line moving." },
  { icon: Camera, title: "Product Launches", description: "Editorial-grade booths and digital sharing tools to amplify launch-day buzz." },
];
...
export default function ServicesPage() {
  useDocumentMeta({
    ...
    description: "Photo booth services for weddings, corporate events, brand activations, birthdays, baby showers and more in Los Angeles.",
  });
      ...
      <section className="px-6 py-24 bg-card/30">
        ...
        <div className="mt-14 max-w-6xl mx-auto grid md:grid-cols-3 gap-6">
          {packages.map((p) => (
            <div
              key={p.name}
              className={p.featured ? "relative bg-gradient-gold text-primary-foreground rounded-3xl p-8 shadow-gold" : "bg-card border border-border rounded-3xl p-8 shadow-soft"}
            >
              {p.featured && (
              ...
              <Link
                to="/contact"
                className={p.featured ? "mt-8 inline-flex w-full justify-center bg-background text-foreground px-6 py-3 rounded-full font-semibold hover:opacity-90" : "mt-8 inline-flex w-full justify-center bg-gradient-gold text-primary-foreground px-6 py-3 rounded-full font-semibold shadow-gold"}
              >
                Book {p.name}
              </Link>
            </div>
